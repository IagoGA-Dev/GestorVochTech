<div>
        ...
        @foreach($bandeiras as $bandeira)
            <li class="py-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700 px-4 rounded-lg transition-colors duration-150">
                ...
                <div class="flex items-center gap-2">
                    <button 
                        x-data
                        @click="$dispatch('open-modal', 'edit-bandeira-{{ $bandeira->id }}')"
    ...
    {{-- Create Modal --}}
    <x-modal name="create-bandeira" focusable>
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">
                Adicionar nova bandeira
            </h2>
            
            @livewire('add-entity', ['entityName' => 'bandeira', 'entityModel' => 'App\Models\Bandeira'])
        </div>
    </x-modal>

    {{-- Edit Modals --}}
    @foreach($bandeiras as $bandeira)
        <x-modal name="edit-bandeira-{{ $bandeira->id }}" focusable>
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">
                    Editar bandeira
                </h2>
                ...
                @livewire('edit-entity', [
                    'entityName' => 'bandeira',
                    'entityModel' => 'App\Models\Bandeira',
                    'entity' => $bandeira
                ], key('edit-bandeira-' . $bandeira->id))
            </div>
        </x-modal>
    @endforeach
</div>
