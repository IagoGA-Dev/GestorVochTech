<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8 max-w-full">
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg rounded-lg group hover:scale-105 transition-transform duration-300">
            <div class="p-4 lg:p-6 flex items-center space-x-3 lg:space-x-4">
                ...
                <div>
                    <div class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-0.5 lg:mb-1">
                        Grupos Econômicos
                    </div>
    ...
    <script>
        function initCharts() {
            // Usando JSON.parse para garantir que os dados sejam válidos
            const chartData = {
                unidadesPorBandeira: JSON.parse('{!! json_encode($unidadesPorBandeira) !!}'),
                unidadesPorGrupo: JSON.parse('{!! json_encode($unidadesPorGrupo) !!}'),
                colaboradoresPorUnidade: JSON.parse('{!! json_encode($colaboradoresPorUnidade) !!}'),
                colaboradoresPorBandeira: JSON.parse('{!! json_encode($colaboradoresPorBandeira) !!}'),
                colaboradoresPorGrupo: JSON.parse('{!! json_encode($colaboradoresPorGrupo) !!}')
            };
            ...
            // Atualizar os gráficos caso o tema mudar
        ...
        // Inicializa os gráficos quando o Livewire for iniciado
        document.addEventListener('livewire:initialized', initCharts);

        // Reinicializa os gráficos ao navegar de volta para o dashboard
